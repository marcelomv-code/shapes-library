...
- **Next phase:** Phase 18 — `ray develop` (manual, host) + Phase 19 — Acceptance scenarios (manual)
...
| 15 — Temp/deck      | DONE (commit pending on host)                           | (pending commit)    | `src/infra/temp/tempManager.ts` centralises temp file/directory lifecycle: pure `buildTempName` (sanitises prefix, collapses underscore runs, falls back to "tmp" on all-unsafe input, monotonic per-ms counter), `writeTempFile`/`createTempDir` create+track, `trackTemp`/`untrackTemp` tracking hooks, `cleanupTemp` (file OR directory, tolerant), `cleanupAllTemps`, `scheduleCleanup` with injectable `TimerFn` for tests, `getActiveTempCount`. Barrel at `src/infra/temp/index.ts`. Contract suite `tests/infra/temp/tempManager.test.ts` (19 tests, all six `buildTempName` branches + track/untrack/count + writeTempFile buffer+string + createTempDir + cleanupTemp file/dir/absent/race + cleanupAllTemps drain + scheduleCleanup timer injection). `src/generator/pptxGenerator.ts` migrated: `activeTempFiles` Set + local `scheduleCleanup`/`cleanupTempFile`/`cleanupAllTempFiles`/`getActiveTempFilesCount` deleted; `generateShapePptx` now calls `writeTempFile(prefix, "pptx", data)`; `openShapeInPowerPoint` calls `scheduleTempCleanup`. `src/import-library.tsx` migrated: ad-hoc `tmpdir()/libimp_<ts>` leaking directory replaced by `createTempDir("libimp")` with `cleanupTemp` in a try/finally block — closes the one-leaked-dir-per-import hole. compactDeck: `src/domain/powerpoint/parseCompactDeck.ts` pure parser (OK:<slides>\|<bytes>, ERROR-wins-over-OK, malformed/missing-ok branches); contract suite `tests/domain/powerpoint/parseCompactDeck.test.ts` (9 tests). `assets/ps/compact-deck.ps1` (UTF-8 BOM) opens the deck, SaveAs ppSaveAsOpenXMLPresentation (enum 24) into `$TempPath`, emits `OK:<slides>\|<bytes>`. `PowerPointClient.compactDeck(deckPath)` added to the port; Windows adapter moves the temp file over the original (renameSync with cross-volume copy-fallback) and funnels through `parseCompactDeckStdout`; Mac adapter throws platform-unsupported; Mock adapter records calls and has a default `{slideCount:0,bytes:0}` response. `src/infra/powershell/scripts.ts` `PsScriptName` union extended with `"compact-deck"`. **Sandbox tsc: 0 errors.** **Sandbox tests:** 230/230 passing (13 files) in 9.01s, including the 19 new tempManager + 9 new parseCompactDeck tests — +28 net from Phase 14.                                                                                                                                                            |
| 16 — Docs           | DONE (commit pending on host)                           | (pending commit)    | Two new top-level docs under `docs/`: `SECURITY.md` (threat model for pptx inputs, library ZIP imports, PS argument flow; per-Phase defense catalog covering Phase 12 zip-slip/zip-bomb guards, Phase 3/4 PS runner hardening, Phase 15 tempManager lifecycle, Phase 14 redaction rule ordering, Phase 7 strict-TS, Phase 13 CI gates; known gaps and reporting instructions) and `ARCHITECTURE.md` (layer map with dependency-rule arrows, port-adapter table for the three PowerPoint adapters, three end-to-end data-flow diagrams — capture, insert, library import — plus test strategy and "where to change what" matrix). `README.md` gains a "Security and Architecture" section linking to both docs and the CODEOWNERS routing. No source or test changes.                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                    |
| 17 — Local build    | DONE (commit pending on host)                           | (pending commit)    | `npm run build` (`ray build -e dist`) on host PS: compiled 3 entry points (`shape-picker.tsx`, `capture-shape.tsx`, `manage-categories.tsx`), generated the TS definitions, passed the Raycast TypeScript check, and finished with `ready - built extension successfully`. Clean output captured in `.audit/phase17-build.txt`. **Finding:** `src/import-library.tsx` exists as a Raycast command entrypoint (`export default async function ImportLibrary(props: { arguments: Args })`) but is NOT registered in `package.json#commands` — the packager silently skips it, so it is unreachable from the Raycast launcher. The `ImportLibraryForm` path under `src/features/shape-picker/` covers the same user flow (push via `ShapeGridItem`), which is why the omission has not been noticed. Decision needed from the owner: either register `import-library` as a dedicated no-view command in `package.json` (preserves the "paste a zip path as argument" argv flow) or delete the orphan file (`src/import-library.tsx` is the only caller of `assertZipIsSafe` outside `libraryZip.ts`, so the pre-extraction guard path would collapse down to one entry point). Not fixed in Phase 17 because the resolution is a scope decision, not a build blocker.                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                      |
...
| 3.8 Export library (ZIP)                            | PASS   | Three successful exports after the export-library.ps1 hardening (defensive `$env:TEMP` resolution, OK emitted before cleanup, best-effort Remove-Item). Toast shows "Library exported"; zips land at `<libraryRoot>\library_export_<ts>.zip`. Producer also moved off `Compress-Archive` to `[System.IO.Compression.ZipFile]::CreateFromDirectory` so new exports use spec-compliant forward-slash entries. The previously residual `EXPORT-CLEANUP-WARN` (cleanup of a temp tree already released by the runner) is silenced as of Polish-P2 below — cleanup is now `Test-Path`-guarded and the catch block swallows without console noise.                                                                                                                                                           |
...
### Polish-P2 — `EXPORT-CLEANUP-WARN` console noise on export

Observed signature on every export run, after the success line:

```
OK:C:\Users\<REDACTED>\AppData\Local\Raycast\extensions\shapes-library\library_export_<ts>.zip
EXPORT-CLEANUP-WARN: An object at the specified path C:\Users\<REDACTED> does not exist.
```

Source: `assets/ps/export-library.ps1` cleanup block. `Remove-Item -Recurse
-Force -ErrorAction SilentlyContinue` was wrapped in a `try/catch` that
echoed the exception message via `Write-Host` whenever the cmdlet still
emitted a terminating error (PS 5.1 occasionally raises one despite
`SilentlyContinue` when AV holds a lock or the runner has already released
the temp tree). The export was already complete by the time the cleanup
ran — the warning carried no actionable signal and only added noise to
the dev console.

**Resolution (2026-04-25):** the catch branch no longer writes to the
console, and the cleanup is gated by `Test-Path -LiteralPath $tmp` so the
script does not even attempt the removal when the temp tree is already
gone. The OS reaps any residual leaves on its normal schedule. No
behaviour change for callers — `OK:$Dest` is still the only stdout line
on success.
